...
void conv3d_CHx7x7(float *o, const float *i, const float *f,
  ...
  while (n < N) {
    ...
    unsigned int n_;
    asm volatile("vsetvli %0, %1, e32, m2, ta, ma" : "=r"(n_) : "r"(N - n));
...
void conv3d_CHx7x7_block(float *o, const float *i, unsigned int num_rows, const float *f,
                         const unsigned int n_) {

  // Helper variables
  const int lwo = N << 2;
  const int lwi = N + F - 1;
  const int lwi_pad = (N + F - 1) << 2;
  ...
  const float *i_slide_ptr_0;
  const float *i_slide_ptr_1;
  const float *i_slide_ptr_2;
  const float *i_slide_ptr_3;

  // Preload two input rows belonging to channel 0
  asm volatile("vle32.v v0, (%0); add %0, %0, %1" : "+&r"(i__) : "r"(lwi_pad));
  ...
  for (int ch = 0; ch < C; ++ch) {
    ...
    if (ch != C - 1) {
      ...
      asm volatile("vfmacc.vf v22, %0, v12" ::"f"(f0_buf));
      asm volatile("vfmacc.vf v16, %0, v4" ::"f"(f1_buf));
      asm volatile("vle32.v v4, (%0)" ::"f"(i__));
      asm volatile("vfmacc.vf v18, %0, v8" ::"f"(f1_buf));
      asm volatile("vfmacc.vf v20, %0, v8" ::"f"(f0_buf));
      f0_buf = f[0 + base_idx_0];
      asm volatile("vfmacc.vf v16, %0, v8" ::"f"(f2_buf));
  ...
  i__ = i_;
  asm volatile("vle32.v v2, (%0); add %0, %0, %1" : "+&r"(i__) : "r"(lwi_pad));
